  ...
  input.onchange = (event) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (tactic && typeof tactic.name === 'string' && Array.isArray(tactic.frames) && Array.isArray(tactic.paths)) {
          onSuccess(tactic);
        } else {
...
const App: React.FC = () => {
  ...
  const handlePieceMove = useCallback((id: string, position: Position) => {
    ...
    const updatePiece = (setter: React.Dispatch<React.SetStateAction<any[]>>) => 
  ...
  const handleAddPath = useCallback((path: Omit<Path, 'id'>) => {
    let finalPath = path;
    if (isPortrait) {
        // Reverse the transformation for saving state
        finalPath = {
  ...
  const handleGoToFrame = useCallback((frameIndex: number) => {
  ...
  const handleSaveTactic = useCallback((name: string) => {
  ...
  const handleLoadTactic = useCallback((tactic: Tactic) => {
  ...
  const transformedPieces = useMemo(() => {
    ...
    return allPieces.map(p => ({ ...p, position: transformPositionForPortrait(p.position) }));
  }, [allPieces, isPortrait, transformPositionForPortrait]);

  const transformedPaths = useMemo(() => {
    if (!isPortrait) return paths;
    return paths.map(p => ({ ...p, points: p.points.map(transformPositionForPortrait) }));
  ...
  return (
    <div className="relative bg-gray-900 text-white min-h-screen flex flex-col items-center p-2 md:p-4 font-sans">
      ...
      <h1 className="text-2xl md:text-3xl font-bold mb-2 md:mb-4 text-center">Field Hockey Tactic Board</h1>
