...
const Piece: React.FC<PieceProps> = ({ piece, onMove, containerRef, animationSpeed }) => {
  ...
  const baseClasses = 'absolute transform -translate-x-1/2 -translate-y-1/2 rounded-full flex items-center justify-center font-bold shadow-lg select-none touch-none';
  const draggingClasses = isDragging ? 'cursor-grabbing scale-110 z-20' : 'cursor-grab z-10 hover:scale-105';
  ...
  let specificClasses = '';
  if (player) {
    const teamColor = player.team === 'red' ? 'bg-red-600 border-red-800' : 'bg-blue-600 border-blue-800';
