...
const ControlButton: React.FC<React.PropsWithChildren<{
  ...
  const baseClasses = 'p-1.5 md:p-2 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-indigo-500';
    ...
    <div className="w-full max-w-5xl p-1 md:p-2 bg-gray-800 rounded-lg flex flex-wrap gap-2 md:gap-4 items-center justify-center">
      ...
      {isDrawingMode && (
        <>
          ...
          <div className="flex items-center gap-1 md:gap-2 bg-gray-700 rounded-md p-1">
            {STROKE_WIDTHS.map(({ name, width }) => (
              <button
                ...
                className={`px-3 py-1 text-sm font-bold rounded-md transition-colors ${strokeWidth === width ? 'bg-indigo-600 text-white' : 'text-gray-300 hover:bg-gray-600'}`}
          ...
          <ControlButton onClick={undoLastPath} disabled={!canUndo} title="Undo Last Drawing">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l4-4m-4 4l4 4" />
            </svg>
          </ControlButton>

          <ControlButton onClick={clearAllPaths} disabled={!canClear} title="Clear All Drawings">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
